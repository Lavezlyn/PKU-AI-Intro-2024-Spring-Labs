...
def q3(name_score_dict):
    ...
    '''
    res = []
    for student in name_score_dict:
        res.append(Student(student, name_score_dict[student]))
    res.sort(key=Student.getScore, reverse=True)
    return res
...
class Layout:
    ...
    def recover_layoutText(self):
        ...
        """
        # 你的实现代码
        layoutText = [' ' * self.width for _ in range(self.height)]
        for x, y in self.walls:
            layoutText[x] = layoutText[x][:y] + '%' + layoutText[x][y+1:]
        for x, y in self.foods:
            layoutText[x] = layoutText[x][:y] + '.' + layoutText[x][y+1:]
        pac_x, pac_y = self.pacman_pos
        layoutText[pac_x] = layoutText[pac_x][:pac_y] + 'P' + layoutText[pac_x][pac_y+1:]
        return '\n'.join(layoutText)
