...
class Attention(Node):
    ...
    def cal(self, X):
        ...
        softmax_score = np.exp(scores - np.max(scores, axis=-1, keepdims=True))
        softmax_score /= softmax_score.sum(axis=-1, keepdims=True)
        attention_output = softmax_score @ V
...
class ResLinear(Node):
    ...
    def cal(self, X):
        ...
        # 即Linear(X)+X
        self.cache.append(X.copy())
        return np.dot(X, self.params[0]) + self.params[1] + X
    ...
    def backcal(self, grad):
        ...
        # 即Linear(X)+X
        X = self.cache[-1]
        self.grad.append(np.einsum('ijk, ijl->kl', X, grad))
        self.grad.append(np.sum(grad, axis=(0, 1)))
        return grad.dot(self.params[0].T) + grad
...
class LogSoftmax(Node):
    ...
    def cal(self, X):
        # TODO: YOUR CODE HERE
        X_ = X - np.max(X, axis=self.dim, keepdims=True)
        expX = np.exp(X_)
        ret = X_ - np.log(expX.sum(axis=self.dim, keepdims=True))
