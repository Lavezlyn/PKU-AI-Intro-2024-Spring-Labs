...
class MCTSAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState, mcts_time_limit=10):
        ...
        def Selection(cgs, cgstree):
            ...
            '''
            # find the node that is not completely expanded
            while (cgstree.north is not None) or (cgstree.east is not None) or (cgstree.south is not None) or (cgstree.west is not None) or (cgstree.stop is not None):
                children = []
                if cgstree.north is not None:
                    children.append((cgstree.north, "North"))
                if cgstree.east is not None:
                    children.append((cgstree.east, "East"))
                if cgstree.south is not None:
                    children.append((cgstree.south, "South"))
                if cgstree.west is not None:
                    children.append((cgstree.west, "West"))
                if cgstree.stop is not None:
            ...
            # return the corresponding game state and the node itself
            return (cgs, cgstree)
        ...
        def Expansion(cgstree):
            ...
            '''
            # Traverse the legal_actions to find all the children of the current game state tree node
            for action in legal_actions:
                successor = cgstree.statevalue.generatePacmanSuccessor(action)
                new_node = Node((successor, 0, 1))
        ...
        def Backpropagation(cgstree, simulation_score):
            while cgstree.parent is not None:
                ...
                '''
                cgstree.numerator = cgstree.numerator + simulation_score
